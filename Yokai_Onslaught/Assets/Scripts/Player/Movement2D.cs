...
public class Movement2D : MonoBehaviour
{
    ...
    private bool _canMove => !_Wall_Latch;
    ...
    private bool _Wall_Latch => _onWall && !_onGround && Input.GetButton("Wall_Latch") && !_wallRun;
    private bool _wallSlide => _onWall && !_onGround && !Input.GetButton("Wall_Latch") && _rb.velocity.y < 0f && !_wallRun;
    ...
    void Animation()
    {
        if (_isDashing)
        {
            ...
            _anim.SetBool("Wall_Latch", false);
        ...
        else
        {
            ...
            if ((_horizontalDirection < 0f && _facingRight || _horizontalDirection > 0f && !_facingRight) && !_Wall_Latch && !_wallSlide)
            {
                Flip();
            }
            if (_onGround)
            ...
            else
            {
                ...
                if (_Wall_Latch || _wallSlide)
                {
                    _anim.SetBool("Wall_Latch", true);
                    _anim.SetBool("isFalling", false);
                    _anim.SetFloat("verticalDirection", 0f);
                }
